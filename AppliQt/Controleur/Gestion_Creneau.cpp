...
void Creneau::TrouverHoraireRDV(int idRes, int duree, int idClient) {
    ...
    int lastHRdv = LastHRDVClient(idClient);
    if (lastHRdv == NULL) {
        lastHClient.insert(idClient, 0);
        lastHRdv = LastHRDVClient(idClient);
    }
...
RendezVous * Creneau::OrdoRDV(Client * client, int IdRes) {
    RendezVous * rdv;

    TrouverHoraireRDV(IdRes, client->getDureeRDV(), client->getId());
